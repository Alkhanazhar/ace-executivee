  ...
  return (
    <nav
      className={`navbar fixed top-0 left-0 z-50 w-full transition-all duration-400 lg:backdrop-blur-xl lg:backdrop-filter ${
        stickyNav
          ? "bg-white shadow-md transition-all duration-200 dark:bg-darkColor"
          : "bg-transparent shadow-none"
      }`}
    >
      <div className="container flex items-center justify-between h-20 navbar__container ">
        <Link
          to="/"
          className={`md:text-2xl text-xl font-[400] drop-shadow-md  inline-block ${
            !stickyNav ? "text-white" : "text-primaryColor"
          }`}
        >
          {brandName}
        </Link>
        <Link
          ...
          className={`navbar__logo text-5xl text-primaryColor ${
            stickyNav ? "text-white" : "text-white"
          }`}
        >
          <svg
        ...
        >
          ...
          <div
            className={`hidden cursor-pointer p-1 text-[1.3rem] dark:text-white lg:flex  ${
              !stickyNav ? "text-white" : "text-darkColor"
            }`}
            onClick={() =>
